...
def Step02_Brainsmash(input_file_name, permutation_times, data_dir=None, output_dir=None):
    if data_dir is None:
        data_dir = global_settings.data_dir
    if output_dir is None:
        output_dir = global_settings.output_dir
    ...
    start = time.time()
    ans = np.zeros([permutation_times, sample_brainmap.shape[0]])
    knn = 2500
    kwargs = {'ns': 50,
                'knn': knn,
                'pv': 70}
    ...
    pd.DataFrame(surrogate_maps).to_csv(os.path.join(output_dir,f'Step02_BrainSmash_resample_{permutation_times}.csv'))

def Step02_Brainsmash2FG(permutation_times, data_dir=None, output_dir=None):
    if data_dir is None:
        data_dir = global_settings.data_dir
    if output_dir is None:
        output_dir = global_settings.output_dir
    ...
    G = pd.read_csv(f'{output_dir}BrainSmash/Step02_BrainSmash_resample_{permutation_times}.csv',index_col=0)
...
def Step02_Brainsmash2PLSR(n_components, median_index, permutation_times, data_dir=None, output_dir=None):
    if data_dir is None:
        data_dir = global_settings.data_dir
    if output_dir is None:
        output_dir = global_settings.output_dir
    ...
    FG_Perm = pd.read_csv(f'{output_dir}BrainSmash/BrainSmash_Rad4mm_sample317_from17853_resample_{permutation_times}.csv', index_col=0)
...
def Step03_GCIsig(n_components, illustrative, permutation_times, data_dir=None, output_dir=None):
    if data_dir is None:
        data_dir = global_settings.data_dir
    if output_dir is None:
        output_dir = global_settings.output_dir
