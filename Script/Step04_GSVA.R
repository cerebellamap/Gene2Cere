...
for(i in seq(20,80,20)){
  ...
  # GSVA
  for (j in c('GO', 'CT', 'DGN')) { # 'CT', 'KEGG' # j = 'DGN'
    print(j)
  ...
  for (j in c('GO', 'CT','DGN')) { # , 'KEGG', 'CT',
    ...
    DEG_sig <- DEG[DEG$adj.P.Val<0.05 & abs(DEG$logFC) > 0,]
    ...
    if(dim(DEG_sig)[1]!=0){
      ...
      png(file = paste0('GSVA_',j,'_',i,'bin','.png'), width = 4000, height = 3000, units = 'px', res = 300)
